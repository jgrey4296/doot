...
from doot.workflow import TaskName, TaskSpec, ActionSpec
...
def mock_task(name, spec=None, actions:int=1, **kwargs):
    ...
    _add_prop(task_m, "name", TaskName(name))
...
def mock_task_spec(name="agroup::mockSpec", pre=None, post=None, action_count=1, extra=None,  **kwargs):
    ...
    if "sleep" not in extra:
        extra['sleep'] = 0.1
    spec_m = MagicMock(TaskSpec(name=name),
    ...
    spec_m.name = TaskName(name)
    return spec_m

def mock_action_specs(num=1) -> list:
    results = []
    for x in range(num):
        action_spec_m = MagicMock(spec=ActionSpec(),
                                  args=[],
                                  kwargs=ChainGuard())
...
def mock_code_ref(returns=None):
    code_ref_m  = MagicMock(spec=CodeReference())
    code_ref_m.__call__ = MagicMock(return_value=returns)
    return code_ref_m

def mock_param_spec(name, val, type=Any):
    m = MagicMock(spec=ParamSpec(name=name, type=type), default=val, prefix="-")
